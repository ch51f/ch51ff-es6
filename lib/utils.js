...
var _addEvent = function _addEvent(target, event, func) {
...
var _removeEvent = function _removeEvent(target, event, func) {
...
var _fixedWechatTitle = function _fixedWechatTitle(title) {
...
var ClassCore = {
	...
	removeClass: function removeClass(el, cName) {
		...
		if (el.classList) {
			...
			el.className = el.className.replace(new RegExp('(^|\\s)' + className + '(?:\\s|$)', 'g'), '$1').replace(/\s+/g, " ") // multiple spaces to one
			.replace(/^\s*|\s*$/g, ''); // trim the ends
		}
		return el;
	},
	// 判断是否存在某个类
	hasClass: function hasClass(el, cName) {
		if (!cName) return false;
		if (el.classList) {
	...
	// toggle Class
	toggleClass: function toggleClass(el, cName) {
